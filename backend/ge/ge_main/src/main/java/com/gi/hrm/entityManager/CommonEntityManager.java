package com.gi.hrm.entityManager;
...
import com.gi.hrm.service.common.ResultPageResponseService;
...
public class CommonEntityManager {
	...
	protected void setParamsMapping(List<String> listFields, Object requestParams, StringBuilder sqlBuilder,
		...
		for (int i = 0; i < fieldsSize; i++) {
			...
			if (exceptFieldName.size() > 0 && exceptFieldName.indexOf(paramName) > -1) {
				continue;
			}
			...
			if (Objects.nonNull(paramValue)) {
				...
				setParameterList(params[i], paramValue, sqlBuilder);
	...
	protected void setQueryParameters(List<NativeQuery<?>> query, EmployeeListSearchRequest requestParams) {
		Field[] params = requestParams.getClass().getDeclaredFields();
	...
	private void setParameterList(Field field, Object paramValue, StringBuilder sqlBuilder) {
