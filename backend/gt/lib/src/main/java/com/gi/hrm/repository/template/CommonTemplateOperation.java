...
public class CommonTemplateOperation {
    ...
    protected void initTemplate(ReactiveMongoTemplate mongoTemplate) {
        if (Objects.isNull(this.mongoTemplate)) {
            this.mongoTemplate = mongoTemplate;
        }
    }

    protected Query initQueryTemplate(ReactiveMongoTemplate mongoTemplate) {
        this.initTemplate(mongoTemplate);
     ...
     * @author HieuHT
     */
    protected void addCriteriaWhereLike(Query query, String field, String value, boolean caseSensity) {
        if (StringUtils.hasText(value)) {
            if (caseSensity) {
                query.addCriteria(Criteria.where(field).regex(String.format(REGEX_LIKE, value)));
            } else {
                query.addCriteria(Criteria.where(field).regex(String.format(REGEX_LIKE, value), "i"));
            }
        }
    }
     ...
     * @author HieuHT
     */
    protected void addCriteriaWhereEqual(Query query, String field, String value, Class<?> clazz) {
        if (Objects.nonNull(value)) {
            Assert.notNull(clazz, "Convert class cannot be null");
            query.addCriteria(Criteria.where(field).is(convertObjectClass(value, clazz)));
        }
    }
        ...
        if (Objects.nonNull(projectionExtendMapping)) {
            for (var entry : projectionExtendMapping.entrySet()) {
                ...
                switch (value) {
                    ...
                    case AggregationExpression aggExp -> {
    ...
    /**
     * Perform fetch query with pagination
     *
     * @param query query with predefined search criteria
     * @param entityClass class to map result into
     * @param pageNo current page
     * @param pageSize limit in one page
     * @param <T> generic type of the entity class
     * @return Page result
     */
    protected <T> Mono<ResultPageResponse<T>> findWithPagination(Query query,
                                                                 Class<T> entityClass,
                                                                 int pageNo,
                                                                 int pageSize) {
        val pageReq = PageRequest.of(pageNo, pageSize);

        query.with(pageReq);

        val items = mongoTemplate.find(query, entityClass).collectList();
        val count = mongoTemplate.count(query.skip(0).limit(0), entityClass);

        return Mono.zip(items, count)
                .map(zip -> ResultPageResponse.fromPage(new PageImpl<>(zip.getT1(), pageReq, zip.getT2())));
    }

    /**
     * Perform fetch query with pagination, use transformer function to map the entity to DTO
     *
     * @param query query with predefined search criteria
     * @param entityClass class to map result into
     * @param pageNo current page
     * @param pageSize limit in one page
     * @param transformer function to map the fetched entity to dto
     * @param <T> generic type of the entity class
     * @param <S> generic type of the DTO class
     * @return Page result
     */
    protected <T, S> Mono<ResultPageResponse<S>> findWithPagination(Query query,
                                                                    Class<T> entityClass,
                                                                    int pageNo,
                                                                    int pageSize,
        ...
        return Mono.zip(items, count).map(zip ->
                ResultPageResponse.fromPage(new PageImpl<>(zip.getT1(), pageReq, zip.getT2()), transformer));
    }
